...
def main():
    ...
    g = EnhancedGraph()
    g.load_from_cache(vpath, epath, args.years, flip_edges=args.flip_edges)
    
    # Load regions
    print(f"[regions] Loading with enhanced validation from {args.region_cache_dir}/", flush=True)
    g.load_regions_from_cache(args.region_cache_dir)
    
    # Print region stats
    us_count = g.get_region_cardinality("us")
    eu_count = g.get_region_cardinality("eu") 
    cn_count = g.get_region_cardinality("cn")
    print(f"[regions] US={us_count:,} EU={eu_count:,} CN={cn_count:,}", flush=True)
    
    # Print shard info (legacy - actual work is hash partitioned)
    start, end = shard_slice(g.num_vertices(), args.part_id, args.total_parts)
